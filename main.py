...
view_mode = st.sidebar.radio("Choose View", ["Chlorine Decay Calculator", "Reservoirs"])

# --- Reservoir View ---
if view_mode == "Reservoirs":
    st.sidebar.markdown("### Reservoir Selection")
    selected_region = st.sidebar.radio("Select a Region", list(reservoirs_by_region.keys()))
    st.subheader(f"Reservoirs in {selected_region}")
    for reservoir in reservoirs_by_region[selected_region]:
        st.write(f"- {reservoir}")

# --- Chlorine Decay Calculator View ---
else:
    ...
    st.sidebar.markdown("### Select Decay Type")
    decay_choice = st.sidebar.radio("Choose Decay Type", ["Wall Decay", "Bulk Decay"])

    # Main Content: Upload Dataset or Manual Input
